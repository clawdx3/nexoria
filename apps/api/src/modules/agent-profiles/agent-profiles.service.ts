...
export class AgentProfilesService implements OnModuleInit {
  ...
  async create(workspaceId: string | null, dto: CreateAgentProfileDto): Promise<AgentProfileResponseDto> {
    const profile = this.repo.create({
      ...
      runtimeMode: dto.runtimeMode ?? 'native_saas',
      runtimeProvider: dto.runtimeProvider ?? 'pro-agent',
      planTier: dto.planTier ?? 'economy',
      remoteConfig: dto.remoteConfig ?? {},
  ...
  private async ensureBuiltInProfiles(): Promise<void> {
    ...
    for (const desired of builtIns) {
      ...
      const patch = {
        ...
        runtimeMode: (desired as any).runtimeMode ?? 'native_saas',
        runtimeProvider: (desired as any).runtimeProvider ?? 'pro-agent',
        planTier: (desired as any).planTier ?? 'economy',
        remoteConfig: (desired as any).remoteConfig ?? {},
        isBuiltIn: true,
  ...
  private builtInWorkspaceOverride(base: AgentProfile, workspaceId: string, isEnabled: boolean): Partial<AgentProfile> {
    return {
      ...
      runtimeProvider: base.runtimeProvider,
  ...
  private toDto(p: AgentProfile): AgentProfileResponseDto {
    return {
      ...
      runtimeProvider: p.runtimeProvider ?? 'pro-agent',
